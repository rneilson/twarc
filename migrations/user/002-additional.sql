...
CREATE INDEX idx_alias_by_user_by_time ON alias (user_id, time_ms DESC);
CREATE TRIGGER tri_add_alias_on_user_insert
  AFTER INSERT ON user
  WHEN new.screen_name IS NOT NULL
  BEGIN
    INSERT OR REPLACE INTO alias (user_id, screen_name, time_ms)
    VALUES (new.id, new.screen_name, new.time_ms);
  END;
...
CREATE INDEX idx_mention_by_user_by_time ON mention (by_user_id, time_ms DESC);
CREATE INDEX idx_mention_of_user_by_time ON mention (of_user_id, time_ms DESC);
CREATE INDEX idx_mention_reply_by_time
  ON mention (reply_to_user_id, time_ms DESC)
  WHERE reply_to_user_id IS NOT NULL;
CREATE INDEX idx_mention_retweet_by_time
  ON mention (retweeted_user_id, time_ms DESC)
  WHERE retweeted_user_id IS NOT NULL;
CREATE INDEX idx_mention_quote_by_time
  ON mention (quoted_user_id, time_ms DESC)
  WHERE quoted_user_id IS NOT NULL;
CREATE TRIGGER tri_update_mention_on_tweet_timestamp_update
  AFTER UPDATE OF time_ms ON tweet
  WHEN new.time_ms != old.time_ms
  BEGIN
    UPDATE mention SET time_ms = new.time_ms
