...
class BaseDB extends EventEmitter {
  ...
  run_async (gen, { use_conn = null, start_trx = false, delay } = {}) {
    ...
    if (use_conn === false) {
      return iterwait(gen.call(this, this.db), delay);
    }
   ...
   */
  get_config (key, { time = false, trx = null } = {}) {
    ...
    function _getcfg (conn) {
      const columns = time ? 'value, time_ms AS time' : 'value';
      let queries;
  ...
  /**
   *  Updates or inserts key(s)/value(s), returns array of [key, success] pairs
   */
  set_config (value, key = '', { time = false, trx = null } = {}) {
    ...
    function _setcfg (conn) {
      ...
      // Use given timestamp or now
        ...
        const set_time = time ? new Date(time).getTime() : Date.now();
        valfn = ([k, v]) => [k, v, set_time];
      }
...
WHERE key = ?1 AND value != ?2 AND (time_ms IS NULL OR time_ms < ?3)`
        ),
        conn.prepare(
      ...
      .then((statements) => {
        const [ upd, chk, ins ] = statements;

        return Promise.all(entries.map((val) => {
          // Get [k, v, t] array
          const [k, v, t] = valfn(val);
          const kvt = [k, JSON.stringify(v), isNaN(parseInt(t)) ? null : t];
  ...
  /**
   *  Deletes config key(s)/value(s), returns false, or deleted key/array of keys 
   */
  del_config (key, { time = false, trx = null } = {}) {
    if (!key) {
      return Promise.reject(new Error(`Must specify key to be deleted`));
    }

    return trx
      ? _delcfg.call(this, trx)
      : this.db.transaction(_trx => _delcfg.call(this, _trx));
      ...
      .then(([ exact, prefixed ]) => {
        ...
        if (exact) {
          return (!del_time || exact.time <= del_time)
            ? conn.run(`DELETE FROM config WHERE key = ?`, [key])
                .then(postfn)
                .then(chg => chg ? key : false)
            : false;
        }
        ...
        // Delete, then return false or array of deleted keys
        return conn.run(
          `DELETE FROM config WHERE key LIKE (? || '.%')`,
          [key]
        )
        .then(postfn)
        .then(chg => chg ? prefixed.map(row => row.key) : false);
      });
    }
  }
