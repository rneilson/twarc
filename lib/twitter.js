			...
			switch(arg.type) {
				...
				case 'other_tweet':
					...
					if (!filters.is_blocked(arg.data.user)) {
						...
						[tweet, user, names] = updateuser(arg.data);
...
function updateuser (source) {
	let [tweet, user, names] = Filters.splituser(source);
	let namedata = [];
	let userdata;
...
function* gettimeline (results, type, since_id, max_id, count) {
	...
	while (!results.complete && api.requests.timeline[type] > 0 && !results.error) {
		...
		let params = {user_id: appcfg.user.id_str};
...
function* refreshtimelines (...types) {
	...
	while (!complete) {
		...
		if (!complete) {
			...
			mgd.log(`Refresh incomplete, resuming at ${new Date(api.nextrefresh.value).toLocaleTimeString()}...`);
			yield api.nextrefresh.then();
		}
	}

	// While we're waiting for writer proc to sync, flush cache of tweets from previous refresh cycle
	...
	mgd.log(`Refresh complete, next refresh at ${new Date(api.nextrefresh.value).toLocaleTimeString()}`);
	return Date.now();
...
function get_userset (type) {
	return getcursored([], 'ids', api.paths.userset[type], {stringify_ids: true}).then(
...
function streamping () {
	...
	streamretry.pingout = setTimeout(() => {
		if ((Date.now() - streamretry.lastping) > streamretry.maxping && !streamretry.timeout) {
			twistream.destroy();
			retrystream();
		}
	}, streamretry.maxping);
...
function startstream () {
	...
	twistream = twit.stream('user', streamparams)
		...
		.on('data', data => writefn(...filters.parse_tweet(data)));
}
