...
function startstream () {
	// Load following list from db
	if (filters.following.size === 0) {
		let list = dbr.getstatus('following');
		if (list !== null) {
			filters.following = new Set(JSON.parse(list));
		}
	}
	...
	twistream = twit.stream('user', streamparams)
		...
		.on('friends', fri => {
			let time = Date.now();
			let update = false;
			let friends = fri.friends_str;
			let friendset;

			// Check size first
			if (filters.following.size !== friends.length) {
				// Build replacement set, skip cross-check
				friendset = new Set(friends);
				update = true;
			}
			else {
				// Build new set
				...
				// Check for new followed ids and build new set
				for (let friend of friends) {
					// Add to new set (for reverse check and/or set replacement)
					friendset.add(friend);

					// Check if not in old set
					if (!filters.following.has(friend)) {
						update = true;
					}
				}

				// Reverse check (ids unfollowed since last update)
				if (!update) {
					for (let friend of filters.following) {
			...
			if (update) {
				// Update friend set
				filters.following = friendset;

				// Write updated list (can reuse already-present array)
				writefn({
					type: 'log',
					...
					type: 'status',
					data: {
						following: JSON.stringify(friends),
						time
					}
				});
			}
