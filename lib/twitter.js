...
function* refreshtimelines (...types) {
	...
	while (!complete) {
		...
		api.reftime = Date.now() + api.window;
		api.requests = _.cloneDeep(api.limits);
		...
		if (!complete) {
			...
			let delaytime = api.reftime - Date.now();
			if (delaytime < 0) {
				delaytime = 0;
			}
			mgd.log(`Refresh incomplete, resuming at ${new Date(api.reftime).toLocaleTimeString()}...`);
	...
	mgd.log(`Refresh complete, next refresh at ${new Date(api.reftime).toLocaleTimeString()}`);
