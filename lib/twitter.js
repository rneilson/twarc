...
// Various functions
function loadsets () {
	// TODO: parameterize to loadsets(...sets)
	...
	// TODO: parameterize to getset(set)
		...
		if (max_id && new_since_id == max_id) {
			...
			for (let tweet of timeline) {
				...
				if (!max_id || Filters.compareid(tweet.id_str, max_id) < 0) {
					...
					let reply = filters.check_reply(tweet);
					if (reply && !cache.tweets.has(reply)) {
						results.replies.add(reply)
					}
...
function* gettimeline (results, type, since_id, max_id, max_requests, count, delay) {
	...
	if (results.complete || !max_requests || results.error) {
		return results;
	}

	// Move down max_id to current since_id and get next batch
	return iterwait(gettimeline(results, type, since_id, results.since_id, max_requests, count, delay), delay);
}
...
function* refreshtimelines (...types) {
	...
	while (!complete) {
		...
		for (let type of types) {
			...
			if (result.error) {
				let e = result.error;
				mgd.err(`Error refreshing timeline '${type}': ${e.message || e}`);
			}
		...
		mgd.log('Refreshed timelines: ' +
			((towrite.length > 0)
			? `updated ${towrite.length} item${towrite.length > 1 ? 's' : ''}`
		...
		if (!complete) {
			...
			mgd.log(`Refresh incomplete, resuming at ${new Date(reftime).toISOString()}...`);
			yield iterwait.delay(delaytime);
		}
	}

	// TODO: fetch replies after
	...
	mgd.log(`Refresh complete, next refresh at ${new Date(delaytime).toISOString()}`);
	iterwait.delay(delaytime).then(() => iterwait(refreshtimelines(), 0));
...
function getblocked () {
	return getcursored([], 'ids', 'blocks/ids', {stringify_ids: true}).then(
		ids => update_userset('blocked', ids),
		err => mgd.err(err)
	);
}
...
function getmuted () {
	return getcursored([], 'ids', 'mutes/users/ids', {stringify_ids: true}).then(
		ids => update_userset('muted', ids),
		err => mgd.err(err)
	);
}

function getfollowing () {
	return getcursored([], 'ids', 'friends/ids', {stringify_ids: true}).then(
		ids => update_userset('following', ids),
		err => mgd.err(err)
	);
}
