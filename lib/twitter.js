...
var twistream = twit.stream('user', params)
	...
	.on('favorite', ev => {
		...
		if (filters.event_user_src(ev) && filters.tweet(ev.target_object)) {
			let source = ev.target_object, target = null;
			let src_type = null, tgt_type = null;

			let src_user = filters.tweetby(source);
			if (filters.cfg[src_user].user_favorited.source) {
				src_type = filters.tweetby(source) === 'by_user' ? 'user_tweet' : 'other_tweet';
			}
	...
	.on('delete', del => {
		...
		writefn({
			...
			data: {
				...
				timestamp_ms: del.status.timestamp_ms
			}
		});
	})
	.on('data', data => {
