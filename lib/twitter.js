...
function* refreshtimelines (...types) {
	...
	while (!complete) {
		...
		if (replies.size > 0) {
			...
			while (replies.size > 0 && max_batches > 0) {
				let tofetch = [];
				let replycount = 0;
				for (let reply of replies) {
					// Check if not in cache
					if (cache.tweets.has(reply.id_str)) {
				...
				while (tofetch.length > 0 && max_batches-- > 0) {
					...
					if (tweets) {
						// Parse next batch
						replyitems = yield iterwait(parsereplies(replies, tweets, replyitems), 0);
						// Remove now-fetched from replies set
						ids.forEach(x => replies.delete(x));
					}
					yield;
				}
			}
		...
		if (!complete) {
			...
			mgd.log(`Refresh incomplete, resuming at ${new Date(reftime).toLocaleString()}...`);
	...
	mgd.log(`Refresh complete, next refresh at ${new Date(reftime).toLocaleString()}`);
