...
function writefn (...args) {
	...
	if (tosend.length > 1) {
		ipc.of.writer.emit('queue', tosend);
	}
	else if (tosend.length == 1) {
		ipc.of.writer.emit(tosend[0].type, tosend[0].data);
	}

	counts.total = tosend.length;
	return counts;
}
...
function unpause () {
	...
	writestream.queue = [];
	if (sent.total > 0 && !writestream.shutdown) {
