...
// Twitter setup
const apiwindow = 15 * 60 * 1000;	// 15 mins
const apimaximums = {
	timeline: {
		user: 72,
		mentions: 12,
...
var apilimits = _.cloneDeep(apimaximums);
const apipaths = {
	timeline: {
...
function* gettimeline (results, type, since_id, max_id, count) {
	...
	while (!results.complete && apilimits.timeline[type] > 0 && !results.error) {
		...
		let timeline;
		try {
			apilimits.timeline[type]--;
			timeline = yield twit.get(apipaths.timeline[type], params);
		}
		catch (e) {
			results.error = e;
			results.complete = false;
...
function* getreplies (replyset) {
	...
	while (replyset.size > 0 && apilimits.statuses.byids > 0) {
		let tofetch = [];
		let replycount = 0;
		...
		while (tofetch.length > 0 && apilimits.statuses.byids > 0) {
			// Get next batch
			let ids = tofetch.splice(0, 100);
			// mgd.log(`Fetching batch of ${ids.length} replies...`);
			let tweets;
			try {
				apilimits.statuses.byids--;
...
function* refreshtimelines (...types) {
	...
	while (!complete) {
		...
		apilimits = _.cloneDeep(apimaximums);
