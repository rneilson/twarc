...
VALUES (?, ?, ?, ?, ?, ?, ?)`
  ...
  get_user_set (type, { time = false, trx = null } = {}) {
    ...
    const users = [];
    const rowfn = time
      ? row => users.push([row.user_id_str, row.since_ms])
      : row => users.push(row.user_id_str);
  ...
  write_user_set (type, user_ids, { time = null, trx = null } = {}) {
    ...
    return this.run_async(
      function* _write_user_set (conn) {
        // Use a set to make checking/removing quicker
        const id_ins = new Set(user_ids);
        // Ids to update can be a simple array
        const id_rem = [];
        ...
        // updated set and if existing time greater, it means
        // nothing to update, but if in db and not in updated
        // set, need to deactivate (by setting until_ms).
        yield conn.each(
          `SELECT CAST(user_id AS TEXT) AS user_id_str, since_ms
FROM ${type} WHERE until_ms IS NULL ORDER BY since_ms DESC`,
          ({ user_id_str, since_ms }) => {
            if (!id_ins.delete(user_id_str) && upd_time > since_ms) {
              id_rem.push(user_id_str);
            }
          }
        );

        // Prepare statements
        const [ insert, remove ] = yield Promise.all([
          conn.prepare(
            `INSERT OR REPLACE INTO ${type} (user_id, since_ms) VALUES (?, ?)`
          ),
          conn.prepare(
            `UPDATE ${type} SET until_ms = ? WHERE user_id = ?`
          ),
        ]);

        // Now insert or update as sorted above
        let added = 0;
        let removed = 0;
        const updates = [];
        const addfn = res => { added += res.changes; };
        const remfn = res => { removed += res.changes; };

        for (const id_str of id_ins) {
          updates.push(insert.run(id_str, upd_time).then(addfn));
        }

        for (const id_str of id_rem) {
          updates.push(remove.run(upd_time, id_str).then(remfn));
        }

        // Wait for all to complete, finalize statements
        yield Promise.all(updates);
        yield Promise.all([insert.finalize(), remove.finalize()]);
  ...
  update_user_set (type, user_id, { mode = 'add', time = null, trx = null } = {}) {
