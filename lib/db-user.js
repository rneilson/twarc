...
class UserDB extends BaseDB {
  ...
  write_item (type, data, { trx = null } = {}) {
    switch (type) {
      ...
      case 'user':
        return this.write_user(data, { trx });
      case 'favorite':
        return this.write_fav(data, { trx });
      case 'unfavorite':
        return this.delete_fav(data, { trx });
  ...
  get_user_by_id (id_str, { trx = null } = {}) {
    ...
    // Fetch user data
    return conn.get(`SELECT json, time_ms FROM user WHERE id = ?`, id_str)
    .then(user_data => {
