...
VALUES (?, ?, ?, ?, ?)`,
      ...
      if (changed && this.id_str === upd_id_str &&
  ...
  write_tweet (tweet, { cache = null, trx = null }) {
    return this.run_async(function* _write_tweet (conn) {
      // Prepare statements
      const use_cache = cache || new StatementCache(conn);
      const { check_time, update, insert } = yield use_cache.load(
        {
          check_time: `SELECT time_ms FROM tweet WHERE id = ?`,
          update: `UPDATE tweet SET time_ms = ?2, json = ?3 WHERE id = ?1`,
          insert: `INSERT
INTO tweet (
  id,
  user_id,
