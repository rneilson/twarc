...
class UserDB extends BaseDB {
  ...
  write_item (type, data, { trx = null } = {}) {
    switch (type) {
      case 'queue':
        return this.write_queue(data, { trx });

      case 'user_tweet':
      case 'other_tweet':
        return this.write_tweet(data, { trx });
      case 'delete':
        return this.delete_tweet(data, { trx });

      // Extra checks for compatibility until changeover complete
      case 'user':
        return this.write_user(time_ms_check(data), { trx });
      case 'favorite':
        return this.write_fav(tweet_id_check(time_ms_check(data)), { trx });
      case 'unfavorite':
        return this.delete_fav(tweet_id_check(time_ms_check(data)), { trx });

      // Can do the _raw version by setting data.time to true
      case 'config':
        return this.set_config(data.value, data.prefix, { time: data.time, trx });
      // New, replacing setting user sets as config entries
      case 'user_set':
        return this.write_user_set(data.type, data.ids, { time: data.time, trx });

      // Only for compatibility until changeover complete
      case 'status':
        return set_status.call(this, _.omit(data, ['time']), data.time, trx);
      case 'status_raw':
        return set_status.call(this, data, true, trx);
      case 'index_names':
        return this.write_aliases(
          _.toPairs(data).map((k, v) => ({
            user_id_str: v,
            screen_name: k,
            time_ms: ''     // Will write time_ms as null
          })),
          { trx }
        );
      case 'name':
    ...
    function time_ms_check (obj) {
      // Compatibility time -> time_ms check
      if (!_.has(obj, 'time_ms') && _.has(obj, 'time')) {
        const newobj = _.omit(obj, ['time']);
        newobj.time_ms = obj.time;
        return newobj;
      }
      return obj;
    }

    function tweet_id_check (obj) {
      // Compatibility id_str -> tweet_id_str check
      if (!_.has(obj, 'tweet_id_str') && _.has(obj, 'id_str')) {
        const newobj = _.omit(obj, ['id_str']);
        newobj.tweet_id_str = obj.id_str;
        return newobj;
      }
    }
  }

  write_queue (queue, { trx = null } = {}) {
    // Initialize counts
    const changes = {};

    // Capture context
    const _write_queue = (conn) => {
      const t = { trx: conn };

      // Advance over queue
      return iterwait(queue, ({ type, data }) => {
        return this.write_item(type, data, t).then((res) => _upd_chg(res, type));
      })
      .then(() => changes);
    }

    return trx ? _write_queue(trx) : this.db.transaction(_write_queue);

    // Updater function
    function _upd_chg (value, key) {
      // Array results should always be [key, success] pairs
      if (_.isArray(value)) {
        // Pick type to update
        const use_type = config_types.has(key)
          ? 'config'
          : item_types.has(key)
            ? key
            : null;

        // Add keys which were updated to applicable array
        if (use_type) {
          // Create applicable array if required
          changes[use_type] = changes[use_type] || [];
          changes[use_type].push(...value);
          // // Value can be boolean or [added, removed] pair
          // .filter(
          //   ([k, v]) => _.isArray(v) ? (v[0] > 0 || v[1] > 0) : !!v
          // )
        }
        // Else throw?
      }
      else if (_.isObject(value)) {
        // Parse results sub-object the same way
        _.forOwn(value, _upd_chg);
      }
      // Else throw?
    }
  }
...
UNION ALL SELECT time_ms, 1 AS was_deleted FROM deleted WHERE id = ?`,
  ...
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      ...
      if (changed && user_mentions.length) {
        ...
        const tweet_type = user_id_str === this.id_str
          ? 'user_tweet'
          : 'other_tweet';
        return { [tweet_type]: result, alias, mention };
      }
...
FROM ${type} WHERE until_ms IS NULL ORDER BY since_ms DESC`,
      ...
      .then(() => {
        // Short-circuit return if no changes (+/- results)
    ...
    return trx ? _write_user_set(trx) : this.db.transaction(_write_user_set);
  }
...
WHERE user_id = ?1 AND until_ms < ?2`
        ...
        const stmt_rem = ((id_rem.length || id_rem.size) > 0)
          ? conn.prepare(
            `UPDATE ${type} SET until_ms = ?2 WHERE user_id = ?1 AND since_ms < ?2`
          )
          : null;

        const statements = yield Promise.all([stmt_ins, stmt_rep, stmt_rem]);
        ...
        for (const entry of id_add) {
          ...
          const id_str = time === true ? entry[0] : entry;
          const since_ms = time === true ? entry[1] : upd_time;
          // Add query to queue
          updates.push(insert.run(id_str, since_ms).then(({ changes }) => {
            if (changes > 0) {
              added += changes;
            }
            else {
              // Try replacing instead
              return replace.run(id_str, since_ms).then(({ changes }) => {
                if (changes > 0) {
                  added += changes;
                }
              });
        ...
        for (const entry of id_rem) {
          // Ids may be bare or in [id_str, until_ms] pairs
          const id_str = time === true ? entry[0] : entry;
          const until_ms = time === true ? entry[1] : upd_time;
          // Add query to queue
          updates.push(update.run(id_str, until_ms).then(({ changes }) => {
            if (changes > 0) {
              removed += changes;
            }
          }));
