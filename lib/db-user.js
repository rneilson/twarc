...
class UserDB extends BaseDB {
  ...
  write_item (type, data, { trx = null } = {}) {
    ...
    function set_status (value, use_time, use_conn) {
      // Recombine first just to make sure
      const use_value = Array.isArray(value) ? recombine(value) : value;

      // Split into user sets and other config entries
      const set_types = Array.from(user_set_types);
      const user_sets = _.pick(use_value, set_types);
      const other_cfg = _.omit(use_value, set_types);

      const updates = [];
      const promises = [];

      // Update any user sets
      _.forOwn(user_sets, (val, type) => {
        const [user_ids, time] = (use_time === true) ? val : [val, use_time];
        promises.push(
          this.write_user_set(type, user_ids, { time, trx: use_conn })
          .then(res => res.user_set[type])
          .then(({ added, removed }) => {
      ...
      // Update config
      if (!_.isEmpty(other_cfg)) {
        promises.push(
          this.set_config(other_cfg, '', { time: use_time, trx: use_conn })
          .then(res => res.reduce((a, [k, v]) => !!v ? (a.push(k) && a) : a, []))
        );
      }

      return Promise.all(promises).then(() => updates.length ? updates : false);
    }
  }
...
FROM ${type} WHERE until_ms IS NULL ORDER BY since_ms DESC`,
      ...
      .then(() => {
        ...
        if (add.size === 0 && remove.size === 0) {
          return { user_set: { [type]: { added: 0, removed: 0, ignored } } };
        }
...
WHERE user_id = ?1 AND until_ms < ?2`
        ...
        return { user_set: { [type]: { added, removed, ignored } } };
      },
      { use_conn: trx, start_trx: true }
    );
  }
