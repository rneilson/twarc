    ...
    return this.run_async(
      function* _write_queue (conn) {
        ...
        for (const { type, data } of queue) {
          // Try writing item
          const written = yield this.write_item(type, data, { trx: conn });

          if (written) {
            // Fire away
            update_changes(data, type);
          }
        }
        return changes;

        // Updater function
        function update_changes (value, key) {
