...
VALUES (?, ?, ?)`,
  ...
  write_mention (mention, { cache = null, trx = null }) {
    if (!_.isObject(mention) && !_.isArray(mention)) {
      return Promise.reject(new Error(`Invalid mention: ${mention}`));
    }
      ...
      const results = yield Promise.all(mentions.map(
        (m) => {
          ...
          return insert.run([
            ...
            of_user_id_str,
            by_user_id_str,
            reply_to_user_id_str || null,
            quoted_user_id_str || null,
            retweeted_user_id_str || null,
            use_time
          ])
          .then(({ changes }) => [[tweet_id_str, of_user_id_str], !!changes]);
        }
      ));
