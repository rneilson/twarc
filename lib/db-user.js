  ...
  write_user_set (type, user_ids, { time = null, trx = null } = {}) {
    ...
    return this.run_async(
      function* _write_user_set (conn) {
        ...
        const id_add = new Set(user_ids);
        const id_rem = new Set();
        ...
        // nothing to update, but if in db, not in updated
...
FROM ${type} WHERE until_ms IS NULL ORDER BY since_ms DESC`,
          ({ user_id_str, since_ms }) => {
            if (!id_add.delete(user_id_str) && upd_time > since_ms) {
              id_rem.add(user_id_str);
            }
          }
        );

        // Prepare statements
        const [ insert, remove, replace ] = yield Promise.all([
          conn.prepare(
            `INSERT OR IGNORE INTO ${type} (user_id, since_ms) VALUES (?, ?)`
          ),
          conn.prepare(
            `UPDATE ${type} SET until_ms = ?2 WHERE user_id = ?1`
          ),
          conn.prepare(
...
WHERE user_id = ?1 AND until_ms < ?2`
        ...
        for (const id_str of id_add) {
          updates.push(insert.run(id_str, upd_time).then(({ changes }) => {
            if (changes > 0) {
              added += changes;
              id_add.delete(id_str);
            }
          }));
        }

        for (const id_str of id_rem) {
          updates.push(remove.run(id_str, upd_time).then(({ changes }) => {
            if (changes > 0) {
              removed += changes;
              id_rem.delete(id_str);
            }
          }));
        }
        ...
        // If we have any stragglers to add that weren't, we
        // might have to replace existing entries instead
        if (id_add.size > 0) {
          updates = [];

          for (const id_str of id_add) {
            updates.push(replace.run(id_str, upd_time).then(({ changes }) => {
              if (changes > 0) {
                added += changes;
                id_add.delete(id_str);
              }
          ...
          // Wait for stragglers to complete
          yield Promise.all(updates);
        }

        // Finalize statements
        yield Promise.all([insert, remove, replace].map(s => s.finalize()));
