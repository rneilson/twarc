...
class Manager extends EventEmitter {
	...
	// Will emit event with proccess name suffix first, then base event
	emit (event, proc, ...args) {
		...
		// emit with an invalid procname suffix, so check first
		if (checkproc(this, proc)) {
			// Emit suffixed event
			super.emit(event + ':' + proc.name, proc, ...args);
		}
	...
	sendmsg (proc, msg, ...args) {
		...
		return new Promise((resolve, reject) => {
			// Sanity check
			if (!checkproc(this, proc)) {
				reject(new Error(`Can't send message to invalid process '${proc}'`));
			}
			...
			// Log error
			this.err(`Error starting process '${child.name}': ${err}`);
