...
class Manager extends EventEmitter {
	...
	emit (event, proc, ...args) {
		...
		if (event === 'exit' || checkproc(this, proc)) {
			// Emit suffixed event
	...
	// Proc can be a current child process, or a name
	// (not checked, to allow waiting for restarting processes)
	// NOTE: will resolve with array of args, since promises require single value
	waitfor (proc, event, timeout) {
		return new Promise((resolve, reject) => {
			let timeoutObj;
			...
			if (typeof proc !== 'string'){
				if (!checkproc(this, proc)) {
					reject(new Error(`Can't wait for event from invalid process ${proc}`));
				}
				// We actually want to wait for the name, not the object
				proc = proc.name;
			}

			this.once(proc + ':' + event, (...args) => {
				if (timeoutObj) {
	...
	// Returns promise waiting for event from all current children
	waitall (event, timeout) {
		let ret = [];
		for (let name of Object.keys(this.children)) {
			let proc = this.children[name];
			if (proc !== undefined) {
				// DEBUG
				// this.log(`Waiting for '${event}' from '${proc.name}'`);
				ret.push(this.waitfor(proc, event, timeout));
			}
		}
		if (ret.length > 0) {
			return Promise.all(ret);
			...
			let exitfn = (code, signal, launchfile, ...launchargs) => {
				...
				if (launchfile) {
					...
					if (this.options.relaunchtime > 0) {
						this.emit('restart', procname, new Promise((resolve, reject) => {
							setTimeout(resolve, this.options.relaunchtime);
						}).then(() => this.launch(launchfile, ...launchargs)));
					}
					else {
						this.emit('restart', procname, this.launch(launchfile, ...launchargs));
					}
				}
