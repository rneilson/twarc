...
class Manager extends EventEmitter {
	...
	launch (procfile, ...args) {
		...
		let started = proc => {
			...
			let exitfn;
			// Relaunch check
			// Separate functions so we don't keep the launch() args around if not req'd
			if (this.options.relaunch) {
				exitfn = (code, signal) => {
					this.log(`Child process '${proc.name}' exited unexpectedly with ${
						(code !== null) ? 'code: ' + code : 'signal: ' + signal}`);
					this.launch(procfile, ...args);
					// TODO: anything else?
				};
			}
			else {
				exitfn = (code, signal) => {
					this.log(`Child process '${proc.name}' exited with ${
						(code !== null) ? 'code: ' + code : 'signal: ' + signal}`);
					// TODO: anything else?
				}
			}
			...
			// Emit started event
			this.emit(`start:${proc.name}`, proc);
		}
		...
		if (Object.prototype.hasOwnProperty(this.children, usename)) {
			throw new Error(`Error starting process '${usename}': already exists`);
		}

		// Process environment values
		var env = Object.assign({}, useopts.forkopts.env || process.env);
	...
	shutdown (proc) {
		...
		if (proc) {
			...
			// Sanity check
			if (!checkproc(proc)) {
				throw new Error(`Can't shutdown unknown process: '${proc.name || proc}'`);
			}
		...
		// user wants to go that route, they're welcome to...)
			...
			for (let name of Object.keys(this.children)) {
				ret.push = this.shutdown(this.children[name]);
			}
			return ret;
		}
	}
