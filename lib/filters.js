...
class Filters {
	...
	constructor (cfg) {
		...
		this.mentioned = (x => _.has(x, 'entities.user_mentions') && _.some(x.entities.user_mentions, this.user));
	...
	static is_tweet (obj) {
		return ((_.has(obj, 'id_str') && _.isString(obj.id_str)) &&
				(_.has(obj, 'text') && _.isString(obj.text)));
	}

	static is_retweet (obj) {
		return (_.has(obj, 'retweeted_status') && _.isObjectLike(obj.retweeted_status));
	}

	static is_quote (obj) {
		return (_.has(obj, 'quoted_status') && _.isObjectLike(obj.quoted_status));
	}

	static is_quote_of_tweet (obj) {
		return ((_.has(obj, 'is_quote_status') && obj.is_quote_status === true) &&
				(_.has(obj, 'quoted_status_id_str') && _.isString(obj.quoted_status_id_str)));
	}

	static is_reply (obj) {
		return (_.has(obj, 'in_reply_to_user_id_str') && _.isString(obj.in_reply_to_user_id_str));
	}

	static is_reply_to_tweet (obj) {
		return (_.has(obj, 'in_reply_to_status_id_str') && _.isString(obj.in_reply_to_status_id_str));
	}
	...
	static splituser (tweet) {
		...
		if (_.has(tweet.user, 'name') && _.has(tweet.user, 'description')) {
			...
			names.push({screen_name: user.screen_name, id_str: user.id_str});
		}
		if (_.has(tweet, 'entities.user_mentions')) {
			for (let mention of tweet.entities.user_mentions) {
				names.push({screen_name: mention.screen_name, id_str: mention.id_str});
			}
		}
		return [tweet, user, names];
	}
	...
	static equaluser (olduser, newuser) {
		function comp (vold, vnew, key) {
			...
			// Skip the *_count keys (or else we'll update users every time they tweet or fav)
			...
			// Object comp
				...
				let kold = Object.keys(vold);
				let knew = Object.keys(vnew);
				// More keys in new object -> unequal
				if (knew.length > kold.length) {
					return false;
				}
				// Recurse per old-object key
				return _.every(kold, (v, k) => comp(v, vnew[k], k));
			}
		...
		if (oldset.size !== newdata.length) {
			...
			if (!update) {
				for (let item of oldset) {
					if (!newset.has(item)) {
						update = true;
						// This time we can break right away, since we've built our new set already
						break;
					}
				}
			}
	...
	on_rt (source, silent) {
		...
		if (!silent && (src_type !== null || tgt_type !== null || quo_type !== null)) {
			ret.push({
				type: 'log',
				data: `[RT ${source.user.screen_name}] ${this.constructor.format(target, quoted)}`
			});
		}
		if (src_type !== null) {
			ret.push({
	...
	on_reply_target (target, silent) {
		...
		// Check target
		tgt_type = this.tweetby(target) === 'by_user' ? 'user_tweet' : 'other_tweet';
	...
	check_reply (tweet) {
		...
		if (this.constructor.is_reply_to_tweet(tweet) && this.cfg[src_user].reply[tgt_user].target) {
