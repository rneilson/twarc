...
function iterwait (iterable, func, time, inittime) {
	...
	function iterstep (result) {
		...
		// Only update if not undefined to allow storing value
		// between calls to setImmediate/setTimeout
		if (result !== undefined) {
			last.value = result;
		}
			...
			try {
				...
				if (!skip) {
					...
					if (isthenable(val)) {
						...
						skip = true;
						// Clear value in case promise returns undefined
