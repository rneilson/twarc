'use strict';

const _ = require('lodash');
const path = require('path');
const BaseDB = require('./db-base');
const iterwait = require('./iterwait');
const { decompose, recombine } = require('./parsecfg');

class MasterDB extends BaseDB {

  constructor (filename, options) {
    super(filename, {
      create_file: _.get(options, 'create_file', true),
      single_conn: _.get(options, 'single_conn', true),
    });

    this.config = {};
  }

  static open (filename, options) {
    const db_opts = _.pick(options, ['create_file', 'single_conn']);
    const migrate = _.get(options, 'migrate', true);
    const migrate_opts = {
      force: _.get(options, 'force', false),
      migrationsPath: path.join(__dirname, '../migrations/master'),
    };

    let p = new Promise((resolve, reject) => {
      // Event handlers - each will remove the other when called
      let on_open, on_err;
      on_open = (file, base_db) => {
        base_db.removeListener('open:error', on_err);
        resolve(base_db);
      };
      on_err = (err, base_db) => {
        const fn = () => reject(err);
        base_db.removeListener('open:success', on_open);
        base_db.close().then(fn, fn);
      };

      // Create and register event handlers
      new MasterDB(filename, db_opts)
        .once('open:success', on_open)
        .once('open:error', on_err);
    });

    if (migrate) {
      p = p.then(master => master.db.migrate(migrate_opts).then(() => master));
    }

    return p;
  }

  /**
   *  Populates config, logtype caches
   */
  init () {

  }

  /**
   *  Refreshes config cache after fetching value(s)
   */
  get_config (key, times = false, trx = null) {
    return super.get_config(key, times, trx).then((cfg) => {
      // Undefined (not found) config can be skipped
      if (cfg !== undefined) {
        // If not returning times, can directly set config cache
        if (!times) {
          if (key) {
            _.set(this.config, key, cfg);
          }
          else {
            this.config = cfg;
          }
        }
        // If times returned, have to extract and set per k/v
        else {
          for (let entry of decompose(cfg, key)) {
            let [ k, [ v ] ] = entry;
            _.set(this.config, k, v);
          }
        }
      }

      return cfg;
    });
  }

  /**
   *  Refreshes config cache after setting value(s)
   */
  set_config (value, key = '', times = false, trx = null) {
    // Decompose value into [k, v] or [k, [v, t]] arrays
    // Array w/o prefix is assumed to already be decomposed
    const entries = (!key && Array.isArray(value)) ? value : decompose(value, key);

    // Forward to parent class
    return super.set_config(entries, key, times, trx).then((chg) => {
      // Combine changes obj
      const cfg = recombine(chg, key);

      // Set config cache per item
      for (let entry of entries) {
        if (_.get(cfg, entry[0])) {
          _.set(this.config, times ? entry[1][0] : entry[1]);
        }
      }

      return chg;
    });

  }

  /**
   *  Refreshes config cache after deleting value(s)
   */
  del_config (key, times = false, trx = null) {
    return super.del_config(key, times, trx).then((chg) => {
      // Delete config cache
      if (chg) {
        _.unset(this.config, key);
      }
    });
  }

  /**
   *  Fetches user id, name, screen name, db path for display
   */
  user_info () {

  }

  /**
   *  Fetches user id, db_path, access token for launching
   */
  user_auth () {

  }

  /**
   *  Creates new user entry
   */
  new_user () {

  }

  /**
   *  Writes log entry to db according to entry type
   */
  log () {

  }

}

module.exports = MasterDB;
