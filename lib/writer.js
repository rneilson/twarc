...
function writefn (type, data, txn, silent) {
		...
		try {
			...
			for (let i = 0; i < queue.length; i++) {
				...
				if (_.has(item, 'type') && _.has(item, 'data')) {
					...
					if (writefn(item.type, item.data, qtxn, true)) {
						syncq = true;
						if (dblog) {
							count[item.type]++;
						}
			...
			if (dblog) {
				if (syncq) {
					...
					mgd.log(`Wrote queue of ${total} items (${counts.join(', ')})`);
				}
				else {
					mgd.log(`All items in queue already processed`);
				}
			}
		}
		catch (e) {
	...
	try {
		switch (type) {
			case 'queue':
				...
				break;
			case 'user':
				sync = dbw.writeuser(data, txn);
				break;
			case 'user_tweet':
				sync = dbw.writetweet(data, txn);
				break;
			case 'other_tweet':
				sync = dbw.writetweet(data, txn);
				break;
			case 'delete':
				sync = dbw.deletetweet(data, txn);
				break;
			case 'favorite':
				sync = dbw.writefav(data, txn);
				break;
			case 'unfavorite':
				sync = dbw.deletefav(data, txn);
				break;
		}
	...
	// 			mgd.err(err);
	// 		}
	// 		else if (dblog && !silent && total > 0) {
	// 			mgd.log(`Synced ${total} items`);
	// 		}
	// 	});
	// }
