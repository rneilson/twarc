...
function writer (type, data) {
	try {
		let ret = writefn(type, data, !dblog);
		if (dblog) {
			mgd.log(ret);
		}
	}
...
function writefn (type, data, silent, txn) {
	...
	// TODO: move to writer() instead so we can pull the rest into DBWriter
	if (!silent) {
		if (ret) {
			if (_.isBoolean(ret)) {
				ret = `Updated ${type}`;
				...
				ret = retstr + `Updated ${total} items (${counts.join(', ')})`;
			}
			// Any other cases we want to handle?
		}
		else if (type === 'queue') {
			ret = 'All items up-to-date';
		}
	}

	return ret;

	function writeq (queue, qtxn) {
		let retq = false;
		let closetxn = false;
		let count = {
			'status': new Set(),
		...
		try {
			...
			for (let i = 0; i < queue.length; i++) {
				...
				if (_.has(item, 'type') && _.has(item, 'data')) {
					...
					let retw = writefn(item.type, item.data, true, qtxn);
					if (retw) {
						if (_.isArray(retw)) {
