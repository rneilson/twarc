...
ipc.serve(() => {
	...
	ipc.server.on('user', writeuser);
	ipc.server.on('user_tweet', writetweet);
	ipc.server.on('other_tweet', writetweet);
	ipc.server.on('delete', deletetweet);
	ipc.server.on('favorite', writefav);
	ipc.server.on('unfavorite', deletefav);
	mgd.go();
	// mgd.sendmsg('ready');
});
ipc.server.start();

// Don't really need to pause anything - everything else comes to us
// mgd.on('pause', () => mgd.sendmsg('ready'));
