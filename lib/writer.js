...
ipc.serve(() => {
	...
	ipc.server.on('status', (data, socket) => writefn('status', data));
	ipc.server.on('following', (data, socket) => writefn('following', data));
		...
		try {
			...
			if (dblog && !silent) {
				if (retq) {
					...
					mgd.log(`Updated ${total} items (${counts.join(', ')})`);
				}
				else {
					mgd.log(`All items up-to-date`);
				}
			}
		}
		catch (e) {
	...
	try {
		switch (type) {
			case 'queue':
				...
				break;
			case 'status':
				dbw.setstatus('', data, txn);
				break;
			case 'following':
				dbw.setstatus('following', data, txn);
				mgd.log('Updated following');
				break;
			case 'user':
		...
		if (dblog && !silent && ret && type !== 'queue') {
			mgd.log(`Updated ${type}`);
		}
	}
	catch (e) {
		mgd.err(e);
