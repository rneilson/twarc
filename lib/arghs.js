...
Arghs.prototype.exitWithUsage = function (exitstr) {
	process.stderr.write(this._usage + '\n' + (exitstr || '') + ((exitstr && exitstr.endsWith('\n')) ? '' : '\n'));
	process.exit(0);
}
...
Arghs.prototype.parse = function (argv) {
		...
		if (this._options[arg] === OPT_BOOL) {
			...
			if (opt) {
				if (this._options[arg] === OPT_ARRAY) {
					...
					if (this._strict.unknown) {
						this.exitWithUsage('Unknown option: --' + arg);
					}
	...
	if (parsed._.length > 0 && this._strict.unnamed) {
		this.exitWithUsage('Received too many arguments: expected ' + this._named.length + ', got ' + (this._named.length + parsed._.length));
	}
