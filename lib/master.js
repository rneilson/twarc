...
const logfile = fs.openSync(path.resolve(appcfg.logpath, 'master.log'), 'a');
...
// mgr.on('restart', (procname, promise) => {
// 	mgr.sendall('pause');
// 	return Promise.all([mgr.waitall('ready'), mgr.waitfor(procname, 'ready')])
// 		.then(() => mgr.log('All processes ready'))
// 		.then(() => mgr.sendall('ready'))
// 		.then(() => mgr.log('Sent ready signal'))
// 		.catch(e => mgr.err(e));
// });
